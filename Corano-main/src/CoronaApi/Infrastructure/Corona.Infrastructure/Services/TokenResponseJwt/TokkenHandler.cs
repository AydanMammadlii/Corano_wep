...
namespace Corona.Infrastructure.Services.TokenResponseJwt;
...
public class TokkenHandler : ITokenHandler
{
    ...
    public async Task<TokenResponseDTO> CreateAccessToken(int minutes, int refreshTokenMinutes, AppUser appUser, Guid basketId)
    {
        ...
        return new TokenResponseDTO(token, ExpireDate, DateTime.UtcNow.AddMinutes(refreshTokenMinutes),
            refleshToken, appUser.UserName, appUser.Email, appUser.Id,basketId, roles.FirstOrDefault());
    }
